...
pushd ${SRC_DIR}/.build/${CHOST}/build/build-cc-gcc-final/
  ...
  make -C gcc prefix=${PREFIX} \
    install-driver install-cpp install-gcc-ar install-headers \
    install-plugin install-lto-wrapper install-collect2
...
#################
# I cannot get the try except with || to work with strip so commenting out for
# now
# pushd ${PREFIX}
#   _files=$(find . -type f)
